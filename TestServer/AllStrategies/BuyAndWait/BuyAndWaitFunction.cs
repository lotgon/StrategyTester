...
{
    public class BuyAndWaitFunction : CommonStrategyFunction
    {
        static object syn = new object();


        internal BuyAndWaitFunction(InputData inSampleData, DateTime startTime, EstimationFunctionType estFunctionType, Account acc, StratergyParameterRange range)
        ...
        public override double OptimizationFunction(StrategyParameter sParam, string directory)
        {
            ...
            return StandardOptimizationFunction(sParam, directory, new BuyAndWaitStrategy(), new BuyAndWaitConfigReader(), out srResult);
        }
    }
}
